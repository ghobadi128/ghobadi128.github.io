...
Under 60 words body. Quick reference → Revenue hook with data point → Soft CTA. Tone: lighter, shorter, more casual.

EMAIL 3 — Soft Close (Day 7):
Subject: Should I close your file?
Under 70 words body. Acknowledgment → Loss aversion hook → Easy reply CTA ("reply 'ATM'"). No pressure, no guilt.
...
- Operator-to-operator tone — peer, not vendor
- Revenue angle should feel like an afterthought, not a pitch
...
export function buildPrompt(data: DispensaryData): string {
  const dataBlock = `Generate a cold outreach sequence for the following dispensary:

FIRST_NAMES: ${data.first_names || "(none provided)"}
BUSINESS_LEGAL: ${data.business_legal || "(none provided)"}
BUSINESS_DBA: ${data.business_dba}
CITY: ${data.city}
STATE: ${data.state || "California"}
ZIP: ${data.zip || "(none provided)"}
ADDRESS: ${data.address || "(none provided)"}
LICENSE_STATUS: ${data.license_status}
LICENSE_TYPE: ${data.license_type}
LICENSE_DESIGNATION: ${data.license_designation}
ISSUE_DATE: ${data.issue_date || "(none provided)"}
EXPIRATION_DATE: ${data.expiration_date || "(none provided)"}
BUSINESS_STRUCTURE: ${data.business_structure}
OWNER_NAMES: ${data.owner_names || "(none provided)"}
